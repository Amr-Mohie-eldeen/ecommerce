...
_redis = None
_hit_counter = (
    Counter("orders_cache_hits_total", "Cache hits", ["cache"]) if Counter else None
)
_miss_counter = (
    Counter("orders_cache_misses_total", "Cache misses", ["cache"]) if Counter else None
)
_latency_hist = (
    Histogram("orders_cache_latency_seconds", "Cache op latency", ["op", "cache"])
    if Histogram
    else None
)
